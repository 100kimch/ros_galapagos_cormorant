...
def labeling(selected=None):
    ...
    if selected:
        ...
        num_files = 0
        for i in get_file_names(p_img):
            if len(i.split("-")) == 2:
                for j in get_file_names(p_img + i + "/labeled"):
            ...
            for j in get_file_names(p_img + i + "/labeled"):
                ...
                if selected:
                    if not num_progress in selected:
                        continue
                    num_selected_progress += 1
                    percentage = int((num_selected_progress / num_files) * 20 + 1)
                    print(
                ...
                info["num_barcodes"] = len(barcodes)
                ...
                if selected:
                    print("  with {:d} barcodes".format(len(barcodes)), end="\r")
